require "tempfile"
require "utils/shell"
require "os/cygwin/diagnostic"
require "os/cygwin/dll"
...
module Homebrew
  module Diagnostic
    class Checks
      ...
      def check_dll_minimum_version
        return unless OS::Cygwin::DLL.below_minimum_version?

        <<~EOS
          Your Cygwin DLL #{OS::Cygwin::DLL.version} is too old.
          We only support kernel #{OS::Cygwin::DLL.minimum_version} or later.
          You will be unable to use binary packages (bottles).
          #{please_create_pull_requests}
          We recommend updating to a newer version Cygwin's installer.
        EOS
      end
    end
  end
