#: @hide_from_man_page
#:  * `migrate-formula-pr` [`--dry-run`] [`--local`] [`--tap=<tap-name>`]  <formula-name>:
#:    Creates a pull request to migrate a formula.
#:
...
module Homebrew
  ...
  def migrate_formula_pr
    local_only = ARGV.include?("--local")
    formula = ARGV.formulae.first
    target = ARGV.value("tap")
    target ||= "core"
    start_point = ARGV.value("start-point")
    ...
    target_tap = Tap.fetch("homebrew", target)
    tap_migrations_path = formula.tap.path/"tap_migrations.json"
    if ARGV.dry_run?
      puts "brew update"
      puts "brew tap #{target_tap.name}"
      puts "cd #{formula.tap.path}"
      cd formula.tap.path
      puts "cp #{formula_relpath} #{target_tap.formula_dir}"
      ...
      safe_system HOMEBREW_BREW_FILE, "tap", target_tap.name
      cd formula.tap.path
      cp formula_relpath, target_tap.formula_dir
      ...
      tap_migrations[formula.name] = target_tap.name
    ...
    branch = "#{formula.name}-#{target}"
    if ARGV.dry_run?
      puts "cd #{formula.tap.path}"
      puts "git checkout --no-track -b #{branch} #{start_point}"
      puts "git commit --no-edit --verbose --message=\"#{formula.name}: migrate to #{target}\" -- #{formula_relpath} #{tap_migrations_path.basename}"
      ...
      unless local_only
        ...
        puts "hub pull-request -m $'#{formula.name}: migrate to #{target}\\n\\nCreated with `brew migrate-formula-pr`.'"
      end
    else
      cd formula.tap.path
      safe_system "git", "checkout", "--no-track", "-b", branch, start_point
      safe_system "git", "commit", "--no-edit", "--verbose",
        "--message=#{formula.name}: migrate to #{target}",
      ...
      unless local_only
        ...
        pr_message = <<-EOS.undent
          #{formula.name}: migrate to #{target}
