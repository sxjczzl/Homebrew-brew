...
module Homebrew
  ...
  class Step
    ...
    def puts_command
      ...
      puts "#{Tty.blue}==>#{Tty.highlight} #{@command.join(" ")}#{Tty.reset}"
    end
    ...
    def puts_result
      if ENV["TRAVIS"]
        ...
        puts "#{Tty.highlight}==>#{Tty.green} PASSED#{Tty.reset}" if passed?
      ...
      puts "#{Tty.highlight}==>#{Tty.red} FAILED#{Tty.reset}" if failed?
    end
  ...
  class Test
    ...
    def skip(formula_name)
      puts "#{Tty.blue}==>#{Tty.highlight} SKIPPING: #{formula_name}#{Tty.reset}"
    end
