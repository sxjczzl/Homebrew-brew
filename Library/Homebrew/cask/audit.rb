...
module Cask
  class Audit
    ...
    def check_untrusted_pkg
      ...
      return unless cask.artifacts.any? { |k| k.is_a?(Cask::Artifact::Pkg) && k.stanza_options.key?(:allow_untrusted) }
    ...
    def check_stanza_requires_uninstall
      ...
      return if cask.artifacts.none? { |k| k.is_a?(Cask::Artifact::Pkg) || k.is_a?(Cask::Artifact::Installer) }
      return if cask.artifacts.any? { |k| k.is_a?(Cask::Artifact::Uninstall) }
    ...
    def check_single_pre_postflight
      ...
      if cask.artifacts.count { |k| k.is_a?(Cask::Artifact::PreflightBlock) && k.directives.key?(:preflight) } > 1
        add_warning "only a single preflight stanza is allowed"
      end

      count = cask.artifacts.count do |k|
        k.is_a?(Cask::Artifact::PostflightBlock) &&
    ...
    def check_single_uninstall_zap
      ...
      if cask.artifacts.count { |k| k.is_a?(Cask::Artifact::Uninstall) } > 1
        add_warning "only a single uninstall stanza is allowed"
      end

      count = cask.artifacts.count do |k|
        k.is_a?(Cask::Artifact::PreflightBlock) &&
      ...
      return unless cask.artifacts.count { |k| k.is_a?(Cask::Artifact::Zap) } > 1
    ...
    def check_generic_artifacts
      cask.artifacts.select { |a| a.is_a?(Cask::Artifact::Artifact) }.each do |artifact|
