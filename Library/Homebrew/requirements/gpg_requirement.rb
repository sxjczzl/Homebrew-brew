...
class GPGRequirement < Requirement
  ...
  def gpg1
    which_all("gpg").detect do |gpg|
      gpg_version = Utils.popen_read(gpg, "--version")[/\d\.\d/, 0]
      next unless gpg_version
      Version.new(gpg_version.to_s) == Version.new("1.4")
    end
  end

  def gpg2
    which_all("gpg2").detect do |gpg2|
      gpg2_version = Utils.popen_read(gpg2, "--version")[/\d\.\d/, 0]
      next unless gpg2_version
      # For now, to give upstreams time to adjust, only support 2.0.x
      # rather than the 2.1.x "modern" series.
      Version.new(gpg2_version.to_s) == Version.new("2.0")
    end
  end
end
